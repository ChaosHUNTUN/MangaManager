...
{
    private static string _defaultDownloadDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "downloads");

    /// <summary>下载根目录，启动时由 Program.cs 注入配置值。设置时自动确保目录存在。</summary>
