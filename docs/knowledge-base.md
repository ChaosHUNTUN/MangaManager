...
- **网格密度调整（按"卡片越小越清晰"）**：`.eh-container .grid` 桌面 `--card-min-width: 200px`（App.css）、移动端竖屏 `104px` + gap `--space-2`（≈3 列，DPR3 下 1.4x 放大，原 4.3x）、横屏 `120px`。实测显示尺寸按 `(可用宽 - 间隙) / 列数` 得出，源图仍为 250px 上限
